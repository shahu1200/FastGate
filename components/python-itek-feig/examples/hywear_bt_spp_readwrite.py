...
def lock_tag(epc, tid, password):
    ...
    print("LOCK=", ret, reader.get_last_error())
...
def update_tag_epc(
    ...
    for pwd in passwords:
        ...
        # read tid memory
        ret = reader.HostMode.read_tid_memory(
            epc,
            tid,
            0,
            6,
        )
        print("tid", ret, reader.get_last_error())
        tid = ret

        # read epc memory
        ret = reader.HostMode.read_epc_memory(epc, tid, 0, 8, pwd)
        print("epc", ret, reader.get_last_error())

        ret = reader.HostMode.read_access_password(epc, tid, pwd)
        print("access", ret, reader.get_last_error())

        ret = reader.HostMode.read_kill_password(epc, tid)
        print("kill", ret, reader.get_last_error())

        # write epc
        print(time.time())
        ret = reader.HostMode.write_epc_memory(epc, tid, sgtin, pwd)
        print(time.time())
        print("write", ret, reader.get_last_error())
    #        if ret is True:
    #            ret = reader.write_access_password()
    #            print('pwd', ret, reader.get_last_error())
    #            lock_tag(newepc, tid, access)
...
while True:
    ...
    eventData = reader._interface.read(timeout=None)
    # print(hexlify(data))

    if eventData[0] == 0x22:  # scan event
        eventData = notif_parser(eventData[1:])

        print("\n>> RECEIVED SCAN EVENT <<", time.time())
        ...
        print("Send output ...")
        ret = reader.set_output([], skipRx=True)
        if ret is True:
            print("... OK")
        else:
    ...
    if state == 0:
        ...
        try:
            ...
            elif eventData["tr_type"] == "BARCODE":
                tagInfo["barcode"] = eventData["idd"]

            else:
                print("Invalid tr_type")
        ...
        ret = reader.adjust_scanmode(idd=True, button=True, scannerId=True)
        if ret is None or ret is False:
            print('... Failed, ', reader.get_last_error())
        else:
            print('... ', ret)
    ...
    if readTag is True:
        ...
        epc = tagInfo["transponder"]["epc"]
        tid = tagInfo["transponder"]["tid"]

        import uuid

        sgtin = uuid.uuid4().hex[0:24]  # random
        print(sgtin)
        update_tag_epc(epc, tid, sgtin)
